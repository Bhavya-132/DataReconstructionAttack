...
for iters in range(300):
    def closure():
        ...
        if memory_buffer:
            mem_gradients = torch.cat([torch.cat([g.view(-1) for g in mem_grad]) for mem_grad in memory_buffer])
    ...
    if len(memory_buffer) < buffer_size:
        memory_buffer.append([g.detach().clone() for g in dummy_dy_dx])
    else:
        memory_buffer[np.random.randint(0, buffer_size)] = [g.detach().clone() for g in dummy_dy_dx]
